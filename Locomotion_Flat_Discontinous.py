...
from launch import LaunchIntrospector, LaunchService
...
def nextstep_update_forward_legs(leg,linear_cmd_vel, desired_cmd_vel, raibert_gain_, polygon_points, front_back_dist, left_right_dist, currentstep):
    ...
    if (is_inside_1 or is_inside_2 or is_inside_3):
        ...
        #history_positions_fl.append(nextstep_fl.copy())  # Store the historical position

    else:
        
        #print("fl(red) is outside the polygon.")

        # Add a while loop after the else condition
        while not (is_inside_1 or is_inside_2 or is_inside_3):
            ...
            #print("raibert_gain_fl", raibert_gain_n_)
...
def nextstep_update_legs(robot_data, leg,linear_cmd_vel, desired_cmd_vel, raibert_gain_, polygon_points, front_back_dist, left_right_dist, currentstep):
    ...
    if (is_inside_1 or is_inside_2 or is_inside_3):
        ...
        #print("br(green) is inside the polygon.")

        # if leg == robot_data.back_right or leg == robot_data.back_left:

        #     while not (min_fb_dist < front_back_dist < max_fb_dist and min_rl_dist < left_right_dist < max_rl_dist):

        #         if front_back_dist > max_fb_dist:
                
        #             raibert_gain_n = raibert_gain_n + 0.05  # Update with your new value

        #         # print("raibert_gain_br",raibert_gain_br)

        #         elif  front_back_dist < min_fb_dist:
        ...
        #         left_right_dist = left_right_dist - 0.9*np.linalg.norm(x)
...
def main():
    ...
    #urdf_file_path = '/home/alinjar/go1_ros2_ws/src/go1_ros2/go1_description/urdf/go1.urdf' #Change with your file path, if required
    urdf_file_path = 'go1.urdf' #Change with your file path, if required
    ...
    node.declare_parameter(linear_cmd_vel_z, 2.5)
    ...
    # print("linear_cmd_vel_x:", linear_cmd_vel_x_value)
    # print("linear_cmd_vel_y:", linear_cmd_vel_y_value)
    # print("linear_cmd_vel_z:", linear_cmd_vel_z_value)
    # print("desired_cmd_vel_x:", desired_cmd_vel_x_value)
    # print("desired_cmd_vel_y:", desired_cmd_vel_y_value)
    # print("desired_cmd_vel_z:", desired_cmd_vel_z_value)
    # print("raibert_gain:", raibert_gain_value)


    # linear_cmd_vel_x_input = float(input("Enter linear_cmd_vel x: "))
    # linear_cmd_vel_y_input = float(input("Enter linear_cmd_vel y: "))
    # desired_cmd_vel_x_input = float(input("Enter desired_cmd_vel x: "))
    # desired_cmd_vel_y_input = float(input("Enter desired_cmd_vel y: "))
    # raibert_gain_input = float(input("Enter raibert_gain_: "))
    ...
    history_positions_torso = []  # Store historical positions for torso
   ...
   # print("torso", torso)
    ...
    currentstep_fl = legframe_fl #initialization

    leg = robot_data.back_right
    legframe_br = leg.leg_frame
    currentstep_br = legframe_br #initialization

    leg = robot_data.front_right
    legframe_fr = leg.leg_frame
    currentstep_fr = legframe_fr #initialization


    leg = robot_data.back_left
    legframe_bl = leg.leg_frame
    currentstep_bl = legframe_bl #initialization
    

    while rclpy.ok():
        # Your existing code goes here

        # Publish any markers or other data
    ...
    #Create Polygon
        ...
        # Example for front_left_leg
        

        aa = 0.3 #aa = (square_side)/2

        LL = 0.7 #Distance between two steps + side of square 

        polygon_points_1 = [Point(x=-aa, y=-aa, z=0.0),
                          Point(x=-aa, y=aa, z=0.0),
                          Point(x=aa, y=aa, z=0.0),
                          Point(x=aa, y=-aa, z=0.0),
                          Point(x=-aa, y=-aa, z=0.0)] #First and last points should be the same to create a closed polygon
                        
        polygon_points_2 = [Point(x=-aa+LL, y=-aa, z=0.0),
                          Point(x=-aa+LL, y=aa, z=0.0),
                          Point(x=aa+LL, y=aa, z=0.0),
                          Point(x=aa+LL, y=-aa, z=0.0),
                          Point(x=-aa+LL, y=-aa, z=0.0)] #First and last points should be the same to create a closed polygon
        
        polygon_points_3 = [Point(x=-aa+2*LL, y=-aa, z=0.0),
                          Point(x=-aa+2*LL, y=aa, z=0.0),
                          Point(x=aa+2*LL, y=aa, z=0.0),
                          Point(x=aa+2*LL, y=-aa, z=0.0),
                          Point(x=-aa+2*LL, y=-aa, z=0.0)] #First and last points should be the same to create a closed polygon
        ...
        if fl_phase == "stance":  #leg is in stance phase
            ...
            if abs(t- fl_start)<1e-3 or abs(t-fl_end)< 1e-3:
                ...
                nextstep_fl = nextstep_update_legs(robot_data,leg,linear_cmd_vel, desired_cmd_vel, raibert_gain_, polygon_points, front_back_dist, left_right_dist, currentstep_fl)
                               
                nextstep_fl = nextstep_fl
            ...
            coeffs_fl_z = trajectory_generator.cspline_coeff(currentstep_fl[2], nextstep_fl[2], 1*linear_cmd_vel_z_value,  -1*linear_cmd_vel_z_value, tdash, swing_time)
        ...
        if br_phase == "stance":  #leg is in stance phase
            ...
            if abs(t-br_start)< 1e-3 or abs(t-br_end) < 1e-3:
                ...
                nextstep_br = nextstep_update_legs(robot_data, leg,linear_cmd_vel, desired_cmd_vel, raibert_gain_, polygon_points, front_back_dist, left_right_dist, currentstep_br)
            ...
            coeffs_br_z = trajectory_generator.cspline_coeff(currentstep_br[2], nextstep_br[2], 1*linear_cmd_vel_z_value,  -1*linear_cmd_vel_z_value, tdash, swing_time)
        ...
        if fr_phase == "stance":  #leg is in stance phase
            ...
            if abs(t- fr_start)<1e-3 or abs(t-fr_end)<1e-3:
                ...
                nextstep_fr = nextstep_update_legs(robot_data,leg,linear_cmd_vel, desired_cmd_vel, raibert_gain_, polygon_points, front_back_dist, left_right_dist, currentstep_fr)
            ...
            coeffs_fr_z = trajectory_generator.cspline_coeff(currentstep_fr[2], nextstep_fr[2], 1*linear_cmd_vel_z_value,  -1*linear_cmd_vel_z_value, t, swing_time)
        ...
        if bl_phase == "stance":  #leg is in stance phase
            ...
            if abs(t-bl_start)< 1e-3 or abs(t-bl_end)< 1e-3:
                ...
                nextstep_bl = nextstep_update_legs(robot_data, leg,linear_cmd_vel, desired_cmd_vel, raibert_gain_, polygon_points, front_back_dist, left_right_dist, currentstep_bl)  
            ...
            coeffs_bl_z = trajectory_generator.cspline_coeff(currentstep_bl[2], nextstep_bl[2], 1*linear_cmd_vel_z_value,  -1*linear_cmd_vel_z_value, tdash, swing_time)
        ...
        torso_orientation = quaternion_from_axis_angle(np.array([0.0,-1.0,0.0]), 0) 

        torso_ori = torso_orientation

        #torso_ori_matrix = Rotation.from_quat(torso_ori).as_matrix()

        torso_ori_matrix = np.eye(3)
