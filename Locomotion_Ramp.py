...
class LegData:
    ...
    #def print_configs(self):
        #print("gait:", self.name)
        #print("walking height:", self.torso_height)
        #print("swing height:", self.swing_height)
        #print("stance height:", self.stance_height)
        #print("max_linear_vel:", self.max_linear_vel)
        #print("max_angular_vel:", self.max_angular_vel)
        #print("front left leg phase:", self.stance_start[0])
        #print("front right leg phase:", self.stance_start[1])
        #print("back left leg phase:", self.stance_start[2])
        #print("back right leg phase:", self.stance_start[3])
        #print("Frequency:", self.frequency)

    def set_type(self, new_gait_name):
        self.name = new_gait_name

    def get_gait_type(self):
        return self.name

    def set_phase(self, phase):
        assert not np.isnan(phase).any(), "Invalid! set_phase input Vector has NaN element"
        self.stance_start = phase

    def set_stance_duration(self, stance_dur):
        self.stance_duration = stance_dur

    def get_stance_duration(self):
        return self.stance_duration

    def get_phase(self):
        return self.stance_start

    def get_walking_height(self):
        return self.torso_height

    def set_walking_height(self, torso_height):
    ...
    def get_stance_height(self):
        return self.stance_height

    def set_stance_height(self, stance_height):
        self.stance_height = stance_height

    def set_max_linear_vel(self, max_linear_vel):
        assert not np.isnan(max_linear_vel).any(), "Invalid! set_max_linear_vel input Vector has NaN element"
        self.max_linear_vel = max_linear_vel

    def get_max_linear_vel(self):
        return self.max_linear_vel

    def set_max_angular_vel(self, max_angular_vel):
        assert not np.isnan(max_angular_vel).any(), "Invalid! set_max_angular_vel input Vector has NaN element"
        self.max_angular_vel = max_angular_vel

    def get_max_angular_vel(self):
...
def calculate_transformation_between_frames_new(parent_frame_name, child_frame_name, parent_orientation, child_orientation, root):
    parent = root.find(".//link[@name='{}']".format(parent_frame_name))
    child = root.find(".//link[@name='{}']".format(child_frame_name))
    ...
    while frames:
        ...
        parent_link_name = parent_frame_name
        ...
        # Modify transformation based on parent and child orientations
        transformation_matrix = np.dot(transformation_matrix, extract_transform(joint))
        
        # Apply parent orientation
        transformation_matrix[:3, :3] = np.dot(parent_orientation, transformation_matrix[:3, :3])
        
        # Apply child orientation inverse
        transformation_matrix[:3, :3] = np.dot(np.linalg.inv(child_orientation), transformation_matrix[:3, :3])
        
        # # Apply parent position
        # transformation_matrix[:3, 3] += parent_position
        
        # # Apply child position
        # transformation_matrix[:3, 3] -= child_position
...
def inverseKinematics2Rnew(ee_pos, leg, link_lengths, safety=False):
    #assert len(ee_pos) == 2, "Invalid! In serial 2r Inverse Kinematics, input ee_pos size != 2"
    assert not np.isnan(ee_pos).any(), "Invalid! In serial 2r Inverse Kinematics, input ee_pos has NaN element"
    ...
    theta1 = np.arctan2(ee_pos[0], ee_pos[2])
    
    # Project the end-effector position onto the XZ plane
    r_xz = np.sqrt(ee_pos[0]**2 + ee_pos[2]**2)
    
    # Calculate the distance between the projection of the end-effector
    # onto the XZ plane and the end-effector in 3D space
    d = np.sqrt(r_xz**2 + ee_pos[1]**2)
    
    # Calculate the angles required to form a triangle with sides l1, l2, and d
    alpha = np.arctan2(-ee_pos[1], -r_xz)
    beta = np.arccos((l1**2 + d**2 - l2**2) / (2 * l1 * d))
    
    # Calculate theta2
    theta2 = np.pi - alpha - beta
...
def inverse_kinematics_3R_new(ee_pos, leg, link_lengths):
    """
    Compute the joint angles for a 3R leg given the end effector position (x, y, z)
    ...
    hip_to_foot_distance = np.linalg.norm(ee_pos)

    theta1 = np.arctan2(-z, -y)  #abduction

    theta3 = np.arccos((hip_to_foot_distance**2-l2**2-l3**2)/(2*l2*l3))

    beta1 = np.arcsin(-x/hip_to_foot_distance)

    gamma1 = np.arcsin(l1*np.sin(theta3)/hip_to_foot_distance)

    theta2 = theta3 - (beta1 + gamma1)

    
    
    
    # d = np.sqrt(x**2 + z**2) - l1
    # D = d**2 + y**2
    
    #theta3 = np.arccos((D - l2**2 - l3**2) / (2 * l2 * l3))

    #theta2 = np.arctan2(y, d) - np.arctan2(l3 * np.sin(theta3), l2 + l3 * np.cos(theta3))
...
def angle_corrector(leg, angles=[0,0,0]):
    angles[1] -= 1.5*math.pi; # add offset 
    
    if leg == 'FR' or leg == 'BR':
        theta_1 = angles[0] - math.pi
        theta_2 = angles[1] + 45*math.pi/180 # 45 degrees initial offset
    else: 
        if angles[0] > math.pi:  
            theta_1 = angles[0] - 2*math.pi
        else: theta_1 = angles[0]
        
        theta_2 = -angles[1] - 45*math.pi/180
    
    theta_3 = -angles[2] + 45*math.pi/180

    
    return [theta_1, theta_2, theta_3]

def leg_IK_calc(xyz, linklengths, leg): 
    ...
    a_2 = math.asin(math.sin(phi)*l1/len_A) 
    a_3 = math.pi - a_2 - phi                   
    ...
    j2 = np.array([0,l1*math.cos(theta_1),l1*math.sin(theta_1)])
    j4 = np.array(xyz)
    j4_2_vec = j4 - j2 # vector from j2 to j4
    
    if leg == 'FR' or 'BR': 
        R = theta_1 - phi - math.pi/2
    else: 
        R = theta_1 + phi - math.pi/2
    ...
    b_2 = math.acos((l2**2 + len_B**2 - l3**2) / (2 * l2 * len_B)) 
    b_3 = math.acos((l2**2 + l3**2 - len_B**2) / (2 * l2 * l3))  
    ...
    # calculate joint 3
    j3_ = np.reshape(np.array([l2*math.cos(theta_2),0, l2*math.sin(theta_2)]),[3,1])
    j3 = np.asarray(j2 + np.reshape(np.linalg.inv(rot_mtx)*j3_, [1,3])).flatten()
    
    # calculate joint 4
    j4_ = j3_ + np.reshape(np.array([l3*math.cos(theta_2+theta_3),0, l3*math.sin(theta_2+theta_3)]), [3,1])
    ...
    angles = angle_corrector(leg=leg, angles=[theta_1, theta_2, theta_3])
...
def nextstep_update_legs(leg,linear_cmd_vel, desired_cmd_vel, raibert_gain_, polygon_points, front_back_dist, left_right_dist, currentstep):
    ...
    polygon_points_1 = polygon_points
    ...
    z1_sol = coeff_1[0] + coeff_1[1]*nextstep[0] + coeff_1[2]*nextstep[1]
    ...
    if (is_inside_1):
        ...
        pass #uncomment when the upper part to maintain geometric constraint between steps is commented


    else:

       # print("br(green) is outside the polygon.")  

        while not (is_inside_1):

            raibert_gain_n = raibert_gain_n+ 0.1  # Update with your new value
...
def main():
    ...
    # foot_pos_fl =  np.array([0.0, 0.0, 0.0])
    # foot_pos_br =  np.array([0.0, 0.0, 0.0]) 
    # foot_pos_fr =  np.array([0.0, 0.0, 0.0])
    # foot_pos_bl =  np.array([0.0, 0.0, 0.0])
    ...
    nextstep_fl = np.array([0.0,0.0,0.0])
    ...
    global min_fb_dist, min_rl_dist, max_fb_dist, max_rl_dist

    # min_fb_dist = 0.22; max_fb_dist = 0.43
    # min_rl_dist = 0.05; max_rl_dist = 0.25

#new, but not relevant
    min_fb_dist = 0.18; max_fb_dist = 0.6
    min_rl_dist = 0.05; max_rl_dist = 0.5
    ...
    node.declare_parameter(linear_cmd_vel_x, 0.2)
    node.declare_parameter(linear_cmd_vel_y, 0.0)
    node.declare_parameter(linear_cmd_vel_z, 0.2)
    node.declare_parameter(desired_cmd_vel_x, 0.1)
    ...
    #create Polygon
        ...
        x1a = 0.0; x1b = 5.0; x1c = 5.0; x1d = 0.0
        y1a = 0.0; y1b = 0.0; y1c = 1.0; y1d = 1.0

        cc = 0.5

        dd = 0.2

        y1a = y1a -cc; y1b = y1b -cc; y1c = y1c -cc; y1d = y1d -cc

        x1a = x1a -dd; x1b = x1b -dd; x1c = x1c -dd; x1d = x1d -dd
        ...
        polygon_points_1 = [Point(x=x1a, y=y1a, z=z1a),
                          Point(x=x1b, y=y1b, z=z1b),
                          Point(x=x1c, y=y1c, z=z1c),
                          Point(x=x1d, y=y1d, z=z1d),
                          Point(x=x1a, y=y1a, z=z1a)] #First and last points should be the same to create a closed polygon
        ...
        #torso = (nextstep_fl+nextstep_fr+nextstep_bl+nextstep_br)/4

        torso = (currentstep_fl+currentstep_fr+currentstep_bl+currentstep_br)/4
        ...
        torso[2] = torso_height + (currentstep_fl[2]+currentstep_fr[2]+currentstep_bl[2]+currentstep_br[2])/4



    #Calculating z's for points on slope

    #global W1, z_arr_1, coeff_1, z1_sol
        ...
        if fl_phase == "stance":  #leg is in stance phase
            ...
            if abs(t- fl_start)<1e-3 or abs(t-fl_end)< 1e-3:
                ...
                nextstep_fl = nextstep_update_legs(leg,linear_cmd_vel, desired_cmd_vel, raibert_gain_, polygon_points, front_back_dist, left_right_dist, currentstep_fl)
        ...
        if br_phase == "stance":  #leg is in stance phase
            ...
            if abs(t-br_start)< 1e-3 or abs(t-br_end) < 1e-3:
                ...
                nextstep_br = nextstep_update_legs(leg,linear_cmd_vel, desired_cmd_vel, raibert_gain_, polygon_points, front_back_dist, left_right_dist, currentstep_br)                
            ...
            #     linear_cmd_vel_z_value_new = 0.15
        ...
        if fr_phase == "stance":  #leg is in stance phase
            ...
            if abs(t- fr_start)<1e-3 or abs(t-fr_end)<1e-3:
                ...
                nextstep_fr = nextstep_update_legs(leg,linear_cmd_vel, desired_cmd_vel, raibert_gain_, polygon_points, front_back_dist, left_right_dist, currentstep_fr)
        ...
        if bl_phase == "stance":  #leg is in stance phase
            ...
            if abs(t-bl_start)< 1e-3 or abs(t-bl_end)< 1e-3:
                ...
                nextstep_bl = nextstep_update_legs(leg,linear_cmd_vel, desired_cmd_vel, raibert_gain_, polygon_points, front_back_dist, left_right_dist, currentstep_bl)  
    ...
    # Taking care of Torso orientation

        print("time", t)

        zxz = 0 #Dummy variable
        ...
        # leg Fl
            ...
            ee_pos = base_to_trunk_rot.T @(new_fl_foot_position - fl_hip_pos)
        ...
        # leg BR
            ...
            ee_pos = base_to_trunk_rot.T @(new_br_foot_position - br_hip_pos)
            ee_pos_hom = np.append(ee_pos,1)
            ee_pos = ee_pos_hom[:3]
        ...
        #leg FR
            ...
            ee_pos = base_to_trunk_rot.T @(new_fr_foot_position - fr_hip_pos)
            ee_pos_hom = np.append(ee_pos,1)
            ee_pos = ee_pos_hom[:3]
        ...
        #leg BL
            ...
            ee_pos = base_to_trunk_rot.T @(new_bl_foot_position - bl_hip_pos)
            ee_pos_hom = np.append(ee_pos,1)
            ee_pos = ee_pos_hom[:3]
